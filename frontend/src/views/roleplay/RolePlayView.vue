            ...
            <div class="chat-record-list">
              <div v-for="round in chatRounds" :key="round.round" class="cr-item" @click="openRoundDetail(round)">
                ...
                <div class="cr-badges">
                  ...
                  <span v-if="round.grammar?.errors?.length" class="cr-badge grammar">语法 {{ round.grammar.errors.length }} 个提示</span>
                  <span v-if="round.grammar && !round.grammar.errors?.length" class="cr-badge perfect">语法正确</span>
                  <span v-if="round.fluency?.total != null" class="cr-badge fluency" :style="{ color: roundScoreColor(round.fluency.total) }">流利 {{ round.fluency.total }}</span>
                </div>
          ...
          <el-tab-pane label="流利度" name="fluency">
            <div v-if="selectedRound?.fluency" class="detail-tab-body">
              <div class="detail-fluency-grid">
                <div class="df-item"><div class="df-label">语速</div><div class="df-score" :style="{ color: dimScoreColor(selectedRound.fluency.wpm?.score) }">{{ selectedRound.fluency.wpm?.score }}/25</div><div class="df-detail">{{ selectedRound.fluency.wpm?.value }} wpm</div></div>
                <div class="df-item"><div class="df-label">停顿</div><div class="df-score" :style="{ color: dimScoreColor(selectedRound.fluency.pause_frequency?.score * 5) }">{{ selectedRound.fluency.pause_frequency?.score }}/20</div><div class="df-detail">{{ selectedRound.fluency.pause_frequency?.pauses_per_min }}次/分</div></div>
                <div class="df-item"><div class="df-label">重复</div><div class="df-score" :style="{ color: dimScoreColor(selectedRound.fluency.repetition?.score * 5) }">{{ selectedRound.fluency.repetition?.score }}/20</div><div class="df-detail">{{ (selectedRound.fluency.repetition?.rate * 100).toFixed(0) }}%</div></div>
                <div class="df-item"><div class="df-label">语法</div><div class="df-score" :style="{ color: dimScoreColor(selectedRound.fluency.grammar?.score * 5) }">{{ selectedRound.fluency.grammar?.score || '—' }}/20</div><div class="df-detail" v-if="selectedRound.fluency.grammar?.errors?.length">{{ selectedRound.fluency.grammar.errors.length }} 个错误</div></div>
                <div class="df-item"><div class="df-label">相关性</div><div class="df-score" :style="{ color: dimScoreColor(selectedRound.fluency.relevance?.score / 15 * 100) }">{{ selectedRound.fluency.relevance?.score || '—' }}/15</div><div class="df-detail" v-if="selectedRound.fluency.relevance?.note">{{ selectedRound.fluency.relevance.note }}</div></div>
              </div>
...
.report-card { background: linear-gradient(135deg, #FBF9FF 0%, #FDFBFF 100%); border: 1.5px solid rgba(124,111,247,0.15); border-radius: 20px; padding: 20px 28px; box-shadow: 0 4px 20px rgba(124,111,247,0.1); &--full { grid-column: 1 / -1; margin-bottom: 16px; } }
.card-title { font-size: 18px; font-weight: 800; color: #2D2D4A; margin-bottom: 16px; display: flex; align-items: center; gap: 8px; .card-subtitle { font-size: 12px; font-weight: 400; color: #999; margin-left: 4px; } }
...
.cr-body { flex: 1; display: flex; flex-direction: column; gap: 6px; min-width: 0; }
.cr-bubble { display: flex; align-items: flex-start; gap: 6px;
  .cr-role-icon { font-size: 16px; flex-shrink: 0; margin-top: 1px; }
  .cr-text { font-size: 13px; line-height: 1.5; color: #4A4A5A; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
  &.user .cr-text { color: #5B5B8A; } &.ai .cr-text { color: #8A5B7A; }
}
.cr-badges { display: flex; flex-direction: column; gap: 4px; flex-shrink: 0; min-width: 90px; }
.cr-badge { font-size: 11px; font-weight: 600; &.pron { color: #5AD8A6; } &.grammar { color: #E6A23C; } &.perfect { color: #67C23A; } &.fluency { color: #5B8DEF; } }
.cr-arrow { font-size: 10px; color: #ccc; flex-shrink: 0; transition: transform 0.2s; .cr-item:hover & { color: #7C6FF7; transform: translateX(2px); } }
.suggestions-text { font-size: 15px; font-weight: 600; color: #555; line-height: 1.8; margin: 0; }
...
.detail-perfect { text-align: center; padding: 60px 20px; font-size: 18px; font-weight: 600; color: #67C23A; }
.detail-grammar-card { .dg-corrected { display: flex; gap: 8px; padding: 12px 16px; background: #F0F8FF; border-radius: 10px; margin-bottom: 16px; .dg-label { font-size: 13px; color: #5B8DEF; font-weight: 600; flex-shrink: 0; } .dg-corrected-text { font-size: 14px; color: #4A4A5A; line-height: 1.5; } } }
.dg-errors { display: flex; flex-direction: column; gap: 10px; }
.dg-error-item { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; padding: 10px 14px; background: #FFFBEB; border: 1px solid #FDE68A; border-radius: 10px;
  ...
  .dge-arrow { font-size: 12px; color: #999; } .dge-correction { font-size: 13px; color: #059669; font-weight: 600; background: #D1FAE5; padding: 2px 6px; border-radius: 4px; }
  .dge-type { font-size: 10px; color: #fff; padding: 2px 6px; border-radius: 8px; flex-shrink: 0; } .dge-explain { font-size: 11px; color: #999; width: 100%; margin-top: 4px; }
}
.detail-fluency-grid { display: flex; gap: 16px; flex-wrap: wrap; justify-content: center; }
.df-item { display: flex; flex-direction: column; align-items: center; background: #FAFBFC; border: 1px solid #EBEEF5; border-radius: 12px; padding: 16px 20px; min-width: 100px;
  .df-label { font-size: 12px; color: #999; margin-bottom: 4px; } .df-score { font-size: 22px; font-weight: 800; } .df-detail { font-size: 11px; color: #bbb; margin-top: 4px; }
}
.df-total { text-align: center; margin-top: 20px; font-size: 15px; color: #666; strong { font-size: 20px; } }
