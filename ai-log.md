## 2026-07-06: 简化密码规则 + 严格端隔离 + 测试账号

- 前后端密码验证从 8-32字符+字母数字 简化为 1-128字符无限制，方便测试
- 严格端隔离：TopNavLayout 移除教师/运营入口，AdminLayout 移除切换到学生端
- 路由守卫：非学生角色访问学生页面自动跳转到自己的后台
- 创建三个测试账号：learner/123、teacher/123、admin/123
...
### 运营看板（5项增强）
- 时间范围筛选器（7/30/90天预设 + 自定义日期）
- 内容使用排行柱状图（发音内容 + 对话场景 Top N）
- 转化漏斗图（注册→测评→首次练习→7日留存）
- 学习活跃日报 + CSV 导出
- 修复 TrendPoint 未导入、SQLAlchemy 子查询列名错误

### 教师端
- Dashboard 工作台：班级/学生/待点评/今日活跃 + 班级分布图 + 最近作业
- 学生进度趋势：四维度折线图 + 打卡统计柱状图
- 批量布置作业（多班级一次布置）

### 其他
- 用户反馈悬浮按钮 → 弹窗提交
- 发音评测统一搜索框（DB命中/LLM验证/TTS合成）
- 内容管理分页（每页10条）
- 删除学习小组模块（前后端+测试+种子数据）
- 语法纠错视图重构

Commit: 228607f | Branch: feat/grammar-check-fixes | 42 files (+2058/-1063)
...
| 11:00 | 模块分析 | 完成模块 1（用户注册与多维度画像）三阶段分析：拆解 7 个子功能、绘制 UML 用例图、编写详细规格说明（7 个子功能完整模板）、生成 5 张流程图（注册/登录/画像/强制引导/进度追踪） | DJQ | ✅ |
...
| 14:55 | 模块分析 | 完成模块 2（英语水平智能测评）三阶段分析：拆解 6 个子功能（自适应推送/客观题判分/口语AI评分/进度恢复/报告生成/题库管理）、绘制 UML 用例图（3 角色 + 6 用例 + include/extend 关系）、编写详细规格说明（6 个子功能完整模板，含输入/输出/处理/异常/接口）、生成 3 张流程图（自适应推送引擎/口语录音评分/进度断点恢复）；分支 docs/module-2-analysis | DJQ | ✅ |
| 15:10 | 模块分析 | 完成模块 3（个性化学习路径规划）三阶段分析：拆解 4 个子功能（规则引擎/任务生成推送/用户干预操作/任务记录回溯）、绘制 UML 用例图（2 角色 + 4 用例 + include/extend 关系）、编写详细规格说明（4 个子功能完整模板，含短板优先配比算法/多因子加权公式）、生成 3 张流程图（规则引擎/任务生成推送/用户干预操作）；分支 docs/module-3-analysis | DJQ | ✅ |
| 15:20 | 模块分析 | 完成模块 4（AI 发音评测与纠错）三阶段分析：拆解 5 个子功能（内容选择/录音上传/多维度评分/错误音素纠音/内容库管理）、绘制 UML 用例图（3 角色 + 5 用例 + include/extend 关系）、编写详细规格说明（5 个子功能完整模板，含五维加权算法/DTW音素对齐/纠音模板库）、生成 2 张流程图（语音录制校验/多维度评分引擎）；分支 docs/module-4-analysis | DJQ | ✅ |
| 15:25 | 模块分析 | 完成模块 5（流利度与完整性评估）三阶段分析：拆解 4 个子功能（语音特征采集/多维度评分计算/语法语义分析/汇总报告与历史追踪）、绘制 UML 用例图（2 角色 + 4 用例 + include/extend 关系）、编写详细规格说明（4 个子功能完整模板，含 WPM/停顿/重复率计算+五维加权映射+分段评级）、生成 1 张流程图（流利度多维度评分计算）；分支 docs/module-5-analysis | DJQ | ✅ |
| 15:30 | 模块分析 | 完成模块 6（智能语音对话练习）三阶段分析：拆解 5 个子功能（场景选择与开场/语音对话闭环引擎/上下文记忆管理/难度自适应控制/对话结束评分）、绘制 UML 用例图（2 角色 + 5 用例 + include/extend 关系）、编写详细规格说明（5 个子功能完整模板，含 System Prompt 模板/应急回复库/CEFR 自适应参数表/四维评分加权）、生成 2 张流程图（语音对话闭环引擎/难度自适应控制）；分支 docs/module-6-analysis | DJQ | ✅ |
| 15:35 | 模块分析 | 完成模块 7（AI 语法纠错与润色）三阶段分析：拆解 4 个子功能（语法错误检测定位/地道表达润色建议/纠错结果展示交互/常见错误汇总统计）、绘制 UML 用例图（2 角色 + 4 用例 + include/extend 关系）、编写详细规格说明（4 个子功能完整模板，含 6 类语法错误检测 Prompt/5 类润色方向/改进建议模板库/去重规则）、生成 1 张流程图（语法纠错与润色流程）；分支 docs/module-7-analysis | DJQ | ✅ |
| 15:40 | 模块分析 | 完成模块 8（情景角色扮演）三阶段分析：拆解 4 个子功能（角色场景选择/角色扮演对话引擎/角色贴合度四维评分/角色内容管理）、绘制 UML 用例图（2 角色 + 4 用例 + include/extend 关系）、编写详细规格说明（4 个子功能完整模板，含 3 角色 Prompt 模板/偏离拉回策略/40-25-20-15 四维评分/说明卡结构）、生成 1 张流程图（角色扮演对话与评分流程）；分支 docs/module-8-analysis | DJQ | ✅ |
