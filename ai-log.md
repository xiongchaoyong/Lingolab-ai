...
### 变更内容
1. **关卡通过动画**：通过时弹跳 + 绿色光晕，失败时抖动 + 红色边框
2. **分数递增动画**：提交评分后数字从 0 递增到目标值 (easeOutCubic, 600ms)
3. **积分飞入动画**：右上角飞出 "+20" 浮动文字，向上飘动淡出
4. **积分栏数字跳动**：顶部积分数字实时递增动画
5. **勋章解锁弹窗**：全屏毛玻璃 + 勋章弹跳入场 + 撒花粒子效果
6. **全部通关庆祝**：横幅弹跳 + 星级依次弹出，渐变背景
7. **配音评分动画**：结果区域从下方滑入，评分条过渡动画
8. **勋章墙增强**：已获得勋章浮动 + hover 抬起效果
9. **Store 增强**：新增 `newBadges`、`lastPointsEarned` 动画状态
