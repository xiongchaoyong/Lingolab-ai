"""语音对话 API — 向后兼容 wrapper（自由对话，mode=scene）

所有逻辑已迁移至 voice_chat.py，旧端点通过硬编码 mode="scene" 保持兼容。
"""

import os, io, base64, uuid, tempfile, subprocess, logging, asyncio, json
...
from app.schemas.conversation import (
    ...
    ConversationSpeakResponse, ConversationEndResponse,
)
...
# 复用 voice_chat 的共享状态和辅助函数
from app.api.voice_chat import _sessions, _init_session, _tts_to_base64, SCENE_OPENERS
...
async def conversation_start(
    ...
    opener = SCENE_OPENERS.get(topic, SCENE_OPENERS["self_intro"])
    llm = get_llm_service()
    ai_text = await llm.chat(topic, opener, [], cefr_level, mode=MODE)

    _sessions[session_id]["history"].append({"role": "ai", "text": ai_text})

    logger.info(f"对话开始: session={session_id}, scene={topic}, level={cefr_level}")
    return ConversationStartResponse(session_id=session_id, ai_text=ai_text, ai_audio_base64="")


@router.post("/speak", response_model=ConversationSpeakResponse)
async def conversation_speak(
    session_id: str = Form(...),
    scene: str = Form(default="self_intro"),
    ...
    try:
        ...
        ai_text = await llm.chat(scene, user_text, session["history"][:-1], cefr_level, mode=MODE)
...
async def conversation_stream_start(
    ...
    scene_openers_subset = {k: SCENE_OPENERS[k] for k in ("self_intro", "directions", "shopping", "restaurant")}
    opener = scene_openers_subset.get(topic, scene_openers_subset["self_intro"])
...
# TTS / end / sessions 端点使用与原有逻辑一致，直接从旧文件迁移保持兼容
# 这些端点与前端的 API 调用紧密耦合（URL 前缀），因此保留在各自路由中
...
async def conversation_stream_speak(
    ...
    tmp_path, converted_path = None, None
    ...
    async def generate():
        ...
        try:
            ...
            try:
                ...
                if db_sid:
                    db_msg = ConversationMessage(session_id=db_sid, round_number=session["round"], role="user", content_text=user_text)
            ...
            try:
                ...
                if db_msg_id:
                    try:
                        db.query(ConversationMessage).filter(ConversationMessage.id == db_msg_id).update({"fluency_scores": fluency_algo})
                        db.commit()
                    except Exception as e:
                        db.rollback()
            ...
            full_text = ""
            async for token in llm.chat_stream(scene, user_text, session["history"][:-1], cefr_level, mode=MODE):
                full_text += token
                yield f"data: {json.dumps({'type': 'token', 'content': token})}\n\n"
            ...
            try:
                db_sid = session.get("db_session_id")
                if db_sid:
                    db.add(ConversationMessage(session_id=db_sid, round_number=session["round"], role="assistant", content_text=full_text))
                    db.commit()
            except Exception as e:
                db.rollback()
            ...
            async def _wait_tagged(task, tag):
                try: return tag, await task, None
                except Exception as e: return tag, None, e

            pending = [_wait_tagged(grammar_task, 'grammar'), _wait_tagged(translation_task, 'translation'), _wait_tagged(hint_task, 'hint')]
            for coro in asyncio.as_completed(pending):
                tag, result, error = await coro
                if tag == 'grammar':
                    if error: logger.warning(f"语法纠错失败: {error}")
                    else:
                        ...
                        if db_msg_id and result:
                            try:
                                db.query(ConversationMessage).filter(ConversationMessage.id == db_msg_id).update({"grammar_check": result})
                                db.commit()
                            except Exception as e:
                                db.rollback()
                                logger.warning(f"保存语法纠错失败: {e}")
                elif tag == 'translation':
                    if error: logger.warning(f"翻译失败: {error}")
                    elif result: yield f"data: {json.dumps({'type': 'translation', 'data': result})}\n\n"
                elif tag == 'hint':
                    if error: logger.warning(f"提示生成失败: {error}")
                    elif result:
                        try: hint_zh = await llm.translate_to_chinese(result)
                        except Exception: hint_zh = ''
                        yield f"data: {json.dumps({'type': 'hint', 'data': {'en': result, 'zh': hint_zh}})}\n\n"
        except Exception as e:
...
@router.post("/tts")
async def conversation_tts(text: str = Form(...), voice: str = Form(default="en-US-JennyNeural")):
...
async def conversation_tts_stream(text: str = Query(...), voice: str = Query(default="en-US-JennyNeural")):
    if not text or not text.strip():
        raise HTTPException(status_code=422, detail="文本不能为空")
    text = text.strip()
    if len(text) > 500:
...
async def conversation_end(
    ...
    pronunciation, text_dimensions, text_dimension_details, utterances = [], [], [], []
    overall = 0
    suggestions = ""
    scoring_methodology = ""
    fluency_report = None
    ...
    try:
        ...
        if len(user_messages) >= 1:
            ...
            text_dimension_details = [
                {"label": "语法正确率", "score": text_result.get("grammar", 75), "feedback": text_result.get("grammar_feedback", ""), "strengths": text_result.get("grammar_strengths", ""), "weaknesses": text_result.get("grammar_weaknesses", "")},
                {"label": "词汇丰富度", "score": text_result.get("vocabulary", 75), "feedback": text_result.get("vocabulary_feedback", ""), "strengths": text_result.get("vocabulary_strengths", ""), "weaknesses": text_result.get("vocabulary_weaknesses", "")},
                {"label": "对话参与度", "score": text_result.get("engagement", 75), "feedback": text_result.get("engagement_feedback", ""), "strengths": text_result.get("engagement_strengths", ""), "weaknesses": text_result.get("engagement_weaknesses", "")},
            ]
            suggestions = text_result.get("suggestions", "")
        else:
            text_dimensions = [{"label": "语法正确率", "score": 0}, {"label": "词汇丰富度", "score": 0}, {"label": "对话参与度", "score": 0}]
        ...
        text_avg = sum(d["score"] * text_weights.get(d["label"], 0.33) for d in text_dimensions) if text_dimensions else 0
        ...
        elif text_dimensions: overall = round(text_avg)

        scoring_methodology = f"综合分 = 语音平均分 × {pron_weight:.0%} + 文本平均分 × {text_weight:.0%}\n语音评测（wav2vec2 + GOP 算法）：音素准确度、重音位置、语调曲线、连读表现、节奏感\n文本评测（LLM 评估）：语法正确率、词汇丰富度、对话参与度"
        ...
        try:
            ...
            profile_updater.ingest_conversation_scores(current_user.id, pronunciation, text_dimensions, source_id=0, db=db)
            db.commit()
        except Exception as e:
    ...
    return ConversationEndResponse(
        ...
        text_dimensions=text_dimensions, suggestions=suggestions,
        utterances=utterances, transcript=history,
...
@router.get("/sessions")
async def list_sessions(limit: int = 10, current_user: UserProfile = Depends(get_current_user), db: Session = Depends(get_db)):
    sessions = (
    ...
    return [
        {
            "id": s.id, "session_uuid": s.session_uuid, "scene": s.scene,
            "cefr_level": s.cefr_level, "round_count": s.round_count, "status": s.status,
